...
public class UDPClient extends Config {
    ...
    public void getOnline(String name) throws Exception {
        ...
        for(int i=0;i<5;i++) {
            DatagramPacket packet = new DatagramPacket(onlineMsg.getBytes(), onlineMsg.getBytes().length, InetAddress.getByName("255.255.255.255"), UDPServerPort + i);
            socket.send(packet);
        }
    ...
    public void active() throws Exception {
        ...
        for(int i=0;i<5;i++) {
            DatagramPacket packet = new DatagramPacket(activeMsg.getBytes(), activeMsg.getBytes().length, InetAddress.getByName("255.255.255.255"), UDPServerPort+i);
            socket.send(packet);
        }
    }
    ...
    public void offline() throws IOException {
        ...
        for(int i=0;i<5;i++) {
            DatagramPacket msgPacket = new DatagramPacket(msg.getBytes(), msg.getBytes().length, InetAddress.getByName("255.255.255.255"), UDPServerPort + i);
            socket.send(msgPacket);
        }
        socket.close();
    }
